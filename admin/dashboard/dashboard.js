...
(function () {
  ...
  // Populate sidebar username initial
  const initial = (user?.username || "A").charAt(0).toUpperCase();
  document.getElementById("sb-av-initial").textContent = initial;
  document.getElementById("sb-uname").textContent = user?.username || "Admin";
...
function updateTime() {
  document.getElementById("tbTime").textContent = new Date().toLocaleString(
    "en-US",
    {
      weekday: "long",
      month: "long",
...
//
// Expected response from api.php?action=admin_dashboard:
// {
//   revenue_today, active_orders, pending_count, delivered_today, drivers_online,
//   recent_orders: [{ id, created_at, customer_name, total_amount, status }],
//   weekly_data:   [{ day_name, count, revenue }]
// }
//
// Expected response from api.php?action=admin_menu:
// [{ name, price, image_url, is_popular }]

function loadDash() {
  fetch("../api.php?action=admin_dashboard")
    .then((r) => r.json())
    .then((d) => {
      // ── KPI CARDS ──────────────────────────────────────────────────
      document.getElementById("kpi-revenue").textContent =
        "Rs. " + (d.revenue_today || 0);
      document.getElementById("kpi-active").textContent =
        d.active_orders ?? "0";
      document.getElementById("kpi-pending-delta").innerHTML =
        `<span class="trend-n">${d.pending_count || 0} awaiting prep</span>`;
      document.getElementById("kpi-today").textContent =
        d.delivered_today ?? "0";
      document.getElementById("kpi-drivers").textContent =
        d.drivers_online ?? "0";
      ...
      // ── TRENDING MENU ──────────────────────────────────────────────
      fetch("../api.php?action=admin_menu")
        .then((r) => r.json())
        .then((items) => {
          const trending = items
            .sort((a, b) => (b.is_popular || 0) - (a.is_popular || 0))
            .slice(0, 4);
          document.getElementById("trendingMenus").innerHTML = trending.length
            ? trending
                .map(
                  (item, i) => `
      ...
      if (!orders.length) {
        tbody.innerHTML =
          '<tr><td colspan="4"><div class="empty-state"><p>No orders yet</p></div></td></tr>';
      } else {
                    ...
                    <tr>
                        <td>
                            <div style="display:flex;align-items:center;gap:0.6rem">
                                ...
                                <div>
                                    <div style="font-weight:700;font-size:0.83rem">#${o.id}</div>
                                    <div style="font-size:0.75rem;color:var(--text3)">${new Date(o.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                                </div>
                            </div>
                        </td>
      ...
      document.getElementById("activityFeed").innerHTML = orders.length
        ? orders
            .slice(0, 8)
            .map((o) => {
              const t = new Date(o.created_at).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              });

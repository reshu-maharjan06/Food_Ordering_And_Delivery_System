          ...
          cart.forEach(c => {
              if (!c.image_url || c.image_url === 'undefined') {
                  if (c.name === 'Samay Baji') { 
                      c.image_url = 'assets/img/foods/newari/samay_baji.jpg'; 
                      c.category = 'Newari'; 
...
function flyToCart(originEl, imgUrl) {
  ...
  fly.style.cssText = `
    ...
    background-image:url('${(imgUrl && !imgUrl.startsWith('http') && !imgUrl.startsWith('../') && window.location.pathname.includes('/customer/')) ? '../' + imgUrl : (imgUrl || '')}');
...
const SauniToast = {
  ...
  add(msg, icon='') {
    ...
    el.innerHTML = `<span>${'Added'}</span><span>${msg}</span>${"to the cart"}`;

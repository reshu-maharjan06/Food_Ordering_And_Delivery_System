    ...
    return `
        <div class="order-card">
            <div class="oc-top">
                ...
                <span style="font-size:0.65rem; color:#6b7280; margin-left:8px;">${o.payment_method.toUpperCase()}</span>
                ${o.payment_status === 'paid' ? '<span style="background:#dcfce7; color:#16a34a; padding:2px 8px; border-radius:10px; font-size:0.6rem; margin-left:auto; border:1px solid #bbf7d0; font-weight:700;">PAID</span>' : (o.payment_status === 'failed' ? '<span style="background:#fee2e2; color:#ef4444; padding:2px 8px; border-radius:10px; font-size:0.6rem; margin-left:auto; border:1px solid #fecaca; font-weight:700;">FAILED</span>' : '<span class="oc-time" style="margin-left:auto;">' + time + '</span>')}
            </div>
...
async function moveOrder(id, status) {
    try {
        ...
        try { d = JSON.parse(text); }
            ...
            if (js !== -1 && je !== -1 && je > js) d = JSON.parse(text.substring(js, je+1));
        ...
        alert("An error occurred while updating order status: " + e.message);
    }
}
...
async function confirmAssign() {
    ...
    try {
        ...
        if (d.success) {
            ...
            alert(d.error || 'Failed to assign driver');
        }
    } catch(e) {
        console.error("Confirm Assign Error:", e);
        alert("An error occurred while assigning driver: " + e.message);
    }
}
